...
void setup() {
  ...
  displayTextCenter("Connecting", 1, 0, 0);
  display.display();
  connectToWifi();
...
void loop() {
  ...
  sendRequest();
  // // String payload = "image|This is a native species|Friendly|native";
  // String payload = "image|Halyomorpha halys is an insect in the family Pentatomidae native to China, Japan, Korea, and other Asian regions|Brown Marmorated Stinkbug|invasive";
  // //image, Halyomorpha halys is an insect in the family Pentatomidae, native to China, Japan, Korea, and other Asian regions, Brown Marmorated Stinkbug, invasive
...
void sendRequest() {
  ...
  String getPointURL = "http://172.20.10.2:8005/receiveData"; // hotspot
...
//image|This is a native species|Friendly|native
//image|Halyomorpha halys is an insect in the family Pentatomidae|native to China,Japan,Korea,and other Asian regions|Brown Marmorated Stinkbug|invasive
...
void displayData(String payload) {
  ...
  String imageStringData = payload.substring(0, firstComma); // base64EncodedImageData
  String description = payload.substring(firstComma + 1, secondComma); 
  String title = payload.substring(secondComma + 1, thirdComma);
  String isInvasive = payload.substring(thirdComma + 1);

  displayText(title, 1, 0, 0);
  displayText(description, 1, 0, 20);
